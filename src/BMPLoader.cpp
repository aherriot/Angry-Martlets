...
void BMPClass::allocateMem(){
	...
	delete[] transparencyBytes;
	transparencyBytes = new BYTE[width*height*4];
}
...
BMPError BMPLoad(std::string fname,BMPClass* bmp)
{
	...
	case(24):
		...
		for(x = 0; x < bmp->width*bmp->height; x += 1)			//except the format is BGR, grr
		{
			BYTE temp=bmp->bytes[3*x];
			bmp->bytes[3*x]=bmp->bytes[3*x+2];
			bmp->bytes[3*x+2]=temp;
		}

		// Now we are going to add a transparency channel to the texture.
		// We are copying all the data into a larger byte array,
		// Then we check to see if the colour is cyan or not
		// and set either full transparency or none
		for(x = 0; x < bmp->width*bmp->height; x += 1) {
			bmp->transparencyBytes[4*x] = bmp->bytes[3*x];
			bmp->transparencyBytes[4*x+1] = bmp->bytes[3*x+1];
			bmp->transparencyBytes[4*x+2] = bmp->bytes[3*x+2];

			//if Red == 0, Green == 0xFF, and Blue == 0xFF, (i.e. CYAN) then make transparent
			if (bmp->bytes[3*x] == 0x00 && bmp->bytes[3*x+1] == 0xFF && bmp->bytes[3*x+2] == 0xFF) {
				bmp->transparencyBytes[4*x+3] = 0xFF;

				//we also set the cyan pixels to black,
				//in case the transparency blending isn't perfect,
				//which it isn't.

				bmp->transparencyBytes[4*x+1] = 0x00;
				bmp->transparencyBytes[4*x+2] = 0x00;
			}
			else
				bmp->transparencyBytes[4*x+3] = 0x00;
		}
		break;
...
BMPError BMPLoad(std::string fname){BMPClass* bmp = new BMPClass();return BMPLoad(fname,bmp);}
